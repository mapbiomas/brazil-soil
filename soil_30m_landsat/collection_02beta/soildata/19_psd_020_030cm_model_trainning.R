...
error_metrics <- data.frame(
  target = "log_clay_sand_020_030cm",
  ME = me,
  MAE = mae,
  MSE = mse,
  RMSE = rmse,
...
png(paste0(res_fig_path, "psd_varimp_log_clay_sand_020_030cm.png"),
...
mtext("Variable Importance: log_clay_sand Model (20-30cm)",
...
error_metrics <- rbind(
  error_metrics,
  data.frame(
    target = "log_silt_sand_020_030cm",
    ME = me,
    MAE = mae,
    MSE = mse,
    RMSE = rmse,
...
vars_per_plot <- ceiling(num_vars / 2)
num_plots <- ceiling(num_vars / vars_per_plot)
png(paste0(res_fig_path, "psd_varimp_log_silt_sand_020_030cm.png"),
  width = 480 * 3, height = 480 * 2, res = 72 * 2
)
par(mar = c(5, 10, 2, 2), mfrow = c(1, num_plots))
xlim <- c(0, max(importance$importance))
...
mtext("Variable Importance: log_silt_sand Model (20-30cm)",
  side = 3, line = -2, outer = TRUE, cex = 1, font = 1
)
dev.off()
