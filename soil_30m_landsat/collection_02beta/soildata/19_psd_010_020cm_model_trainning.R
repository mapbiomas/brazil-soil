...
# Read data from disk
soildata <- read_insync("psd_c02beta_010_020cm_v2.csv")
# Check the data
dim(soildata)
# 19944    81
...
soildata <- soildata[
  ,
  c("system:index", "latitude", "longitude", "ndvi", ".geo") := NULL
]
...
# List existing files in the original_path and get the last one. Then read it.
...
error_metrics <- data.frame(
  target = "log_clay_sand_010_020cm",
  ME = me,
  MAE = mae,
  MSE = mse,
  RMSE = rmse,
...
png(paste0(res_fig_path, "psd_varimp_log_clay_sand_010_020cm.png"),
...
mtext("Variable Importance: log_clay_sand Model (10-20cm)",
...
error_metrics <- rbind(
  error_metrics,
  data.frame(
    target = "log_silt_sand_010_020cm",
    ME = me,
    MAE = mae,
    MSE = mse,
    RMSE = rmse,
...
vars_per_plot <- ceiling(num_vars / 2)
num_plots <- ceiling(num_vars / vars_per_plot)
png(paste0(res_fig_path, "psd_varimp_log_silt_sand_010_020cm.png"),
  width = 480 * 3, height = 480 * 2, res = 72 * 2
)
par(mar = c(5, 10, 2, 2), mfrow = c(1, num_plots))
xlim <- c(0, max(importance$importance))
...
mtext("Variable Importance: log_silt_sand Model (10-20cm)",
  side = 3, line = -2, outer = TRUE, cex = 1, font = 1
)
dev.off()
...
write.csv(
  error_metrics,
  file = paste0(res_tab_path, "psd_error_metrics_010_020cm.csv"),
  row.names = FALSE
)
...
for (group in unique(soildata$id)) {
  ...
  print(group)
  # Check if the validation group has any samples with depth 10-20cm
  if (nrow(soildata[id == group & depth > 10 & depth <= 20]) == 0) {
    print("No samples with depth 10-20 cm in group")
  } else {
    print("Samples with depth 10-20 cm in group")
    ...
    # Predict on the validation group, for layers with depth 10-20cm
    soildata[
      id == group & depth > 10 & depth <= 20,
      pred_log_clay_sand :=
      predict(model, data = soildata[id == group & depth > 10 & depth <= 20,
