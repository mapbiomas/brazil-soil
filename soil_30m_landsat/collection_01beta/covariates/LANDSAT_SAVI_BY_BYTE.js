...
var datasets = [ 
  {
    ...
    allProcess:function(col){
      ...
      return ee.ImageCollection(col).map(function(image){
        ...
        image = addBand_SAVI(image);
        return image;
      });
    },
    reduceProcess:function(col){
      var max = col.select('savi').max().rename('savi_max');
      var median = col.select('savi').median().rename('savi_median');
      return max
        .addBands(median)
        .int16();
    },
...
function addBand_SAVI (image){
  var exp = '1.5 * (( b("nir") - b("red")) / ((b("nir") + b("red") + 0.5)))';
  var savi = image
    .expression(exp)
    .add(1)
    .multiply(100)
...
.forEach(function(obj){
  ...
  .forEach(function(year){
    ...
    var output_folder = 'projects/mapbiomas-workspace/SOLOS/COVARIAVEIS/LANDSAT_SAVI_BY_BYTE/';

    Map.addLayer(image.select('savi_mean'), {min:85,max:156}, description, false);
