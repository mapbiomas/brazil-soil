...
file_path <- "res/tab/soc_model03_hyperparameter_tunning.txt"
...
data.table::fwrite(correlation, "res/tab/soc_model03_hyperparameter_correlation.txt", sep = "\t")
...
hyper_best <- data.frame(num_trees = 400, mtry = 16, min_node_size = 2, max_depth = 40)
...
data.table::fwrite(soc_model_stats, "res/tab/soc_model03_model_statistics.txt", sep = "\t")
print(round(soc_model_stats, 2))

# Write model parameters to disk
write.table(capture.output(print(soc_model))[6:15],
  file = "res/tab/soc_model03_model_parameters.txt", sep = "\t", row.names = FALSE
)
if (FALSE) {
  # Read the model parameters from disk
  soc_model <- data.table::fread("res/tab/soc_model03_model_parameters.txt", sep = "\t")
  print(soc_model)
}
...
dev.off()
png("res/fig/soc_model03_variable_importance.png", width = 480 * 3, height = 480 * 3, res = 72 * 2)
...
# ranges from 0 to 1.
color_breaks <- seq(0, ceiling(abs_error_tolerance / 1000), length.out = 5)
...
dev.off()
png("res/fig/soc_model03_observed_versus_oob.png", width = 480 * 3, height = 480 * 3, res = 72 * 3)
